...
static void _c2s_component_presence(c2s_t *c2s, nad_t *nad) {
    ...
    if(nad_find_attr(nad, 0, -1, "type", NULL) < 0) {
        LOG_DEBUG(sess->c2s->log, "component available from '%s'", from);

        LOG_DEBUG(sess->c2s->log, "sm for serviced domain '%s' online", from);
    ...
    LOG_DEBUG(sess->c2s->log, "component unavailable from '%s'", from);

    if(xhash_get(c2s->sm_avail, from) != NULL) {
        LOG_DEBUG(sess->c2s->log, "sm for serviced domain '%s' offline", from);
        ...
        if(xhash_iter_first(c2s->sessions))
            do {
                ...
                if(sess->resources != NULL && strcmp(sess->resources->jid->domain, from) == 0) {
                    LOG_DEBUG(sess->c2s->log, "killing session %s", jid_user(sess->resources->jid));
